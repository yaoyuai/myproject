<template>
	<div class="video-cls">
		...
		<el-table :data="videoInfoList" stripe style="margin-left: 5%; margin-right: 10%; width:80%;">
			...
			<el-table-column prop="trainId" label="训练编号">
			</el-table-column>
			<el-table-column prop="trainName" label="训练名称">
			</el-table-column>
			<el-table-column prop="teacherName" label="上传者">
			</el-table-column>
			<el-table-column prop="trainCtime" label="上传时间"  sortable>
			</el-table-column>
			...
			<el-table-column label="操作">
				<template slot-scope="scope">
					<el-tooltip class="item" effect="dark" content="去训练" placement="top-start">
...
<script>
	...
	export default {
		...
		methods: {
			showFile(index,row){
				sessionStorage.setItem("train",JSON.stringify(row));
				window.open("/#/traindes")
			},
			pageClick(nowPage) {
			...
			getVideoList(){
				...
				axios.post("/train/selecttrainbycourseid",{"courseId":obj.courseId,"nowPage":(this.nowPage-1)*this.divideNum,"divideNum":this.divideNum}).then((resp)=>{
