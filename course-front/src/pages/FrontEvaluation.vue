<template>
	<div class="head">
		<el-input type="textarea" :rows="5" placeholder="请输入评价内容" v-model="evaluationContent" style="width: 80%;;margin-left: 2%;margin-top: 2%;" :disabled="textAreaFlag">
		</el-input>
		<div>——</div>
		...
		<el-rate v-model="evaluationScore" show-text style="margin-left: 85%;margin-top: -3%;">
		</el-rate>
		...
		methods: {
			...
			evaluate(){
				if(this.evaluationContent==""){
					this.commOpen("请输入评价内容");
					return;
				}
				if(this.evaluationScore==0){
					this.commOpen("请在按钮下方进行打分操作");
					return;
				}
				
				
				
				axios.post("evaluation/add",{"courseId":this.courseId,"evaluationContent":this.evaluationContent,"evaluationScore":this.evaluationScore}).then((resp)=>{
					this.commOpen(resp.data.message);
					if(resp.data.id==1){
						this.selectHasEvaluation();
